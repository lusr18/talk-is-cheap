'''
Title: OpenAI Assistant API Python Wrapper
Description: A wrapper for the OpenAI Assistant API to help you build your own AI assistant
'''

from openai import OpenAI
from openai import Client
# Adjust path to call from pages/components/Assistant
...
class Conversation:
    """ A conversation between an assistant and a user """
...
class AIAssistant:
    ...
    def __init__(
        ...
        use_third_party_retrieval: str = None,
        ...
        self.use_third_party_retrieval = use_third_party_retrieval
        ...
        if self.use_third_party_retrieval:
            if self.use_third_party_retrieval == "Milvus":
                pass
            else:
                raise ValueError(f"Third party retrieval {self.use_third_party_retrieval} not supported")
    ...
    def extract_run_message(self, run: Run, thread_id: str) -> str:
        ...
        for message in messages:
            if message.run_id == run.id:
                ...
                return self.format_message(message=message)
    ...
    def delete_assistant(self):
        """ """
        # TODO Only supports deleting one thread for now (from chat create_chat_instance)
    ...
    def create_chat_instance(self):
        """ """
        # TODO Only supports code interpreter for now
        self.thread = self.create_thread()
        
    def ask_chat(self, content: str, file_ids: list[str] = None):
        """ """
        if self.thread == None:
            return Exception("No chat instance created, please run create_chat_instance")
        
        
        message = self.create_response(
            thread_id=self.thread.id, content=content, message_files=file_ids
        )
        return message
    ...
    def chat(self, file_ids: list[str] = None):
        ...
        while user_input != "bye" and user_input != "exit":
            ...
            print(f"\033[33m {message.role}: + {message}")
